...
fn test_creature_finds_and_consumes_food() {
    ...
    assert!(creature.needs.hunger < 0.8, "Creature should have reduced hunger. Final hunger: {}", creature.needs.hunger);
...
fn test_creature_finds_and_consumes_water() {
    ...
    let creature = &sim.world.creatures[&creature_entity];
    assert!(creature.needs.thirst < 0.8, "Creature should have reduced thirst. Final thirst: {}", creature.needs.thirst);
...
fn test_creature_dies_from_starvation() {
    ...
    for i in 0..120 {
        ...
        if i < 5 || (i >= 58 && i <= 62) || i % 60 == 0 {
            println!("Frame {}: health={}, alive={}", i, creature.health.current, creature.is_alive());
        }
    }
...
fn test_creature_dies_from_dehydration() {
    ...
    // Since events are processed during update, check creature state instead
    let creature = &sim.world.creatures[&creature_entity];
    assert!(!creature.is_alive(), "Creature should be dead from dehydration");
}
...
fn test_exhausted_creature_rests() {
    ...
    for i in 0..120 {  // Give more time
        sim.update(1.0 / 60.0);
        
        if i % 30 == 0 {
            let creature = &sim.world.creatures[&creature_entity];
            println!("Frame {}: energy={}, state={:?}", i, creature.needs.energy, creature.state);
        }
    }
    
    // Creature should have rested and recovered energy
    let creature = &sim.world.creatures[&creature_entity];
    assert!(creature.needs.energy > initial_energy, 
            "Creature should have recovered energy. Initial: {}, Final: {}", 
            initial_energy, creature.needs.energy);
}
...
fn test_resource_depletion_event() {
    ...
    for i in 0..300 {
        ...
        if i % 60 == 0 {
            let resource = &sim.world.resources[&food_entity];
            let creature = &sim.world.creatures[&creature_entity];
            println!("Frame {}: resource amount={}, creature hunger={}", i, resource.amount, creature.needs.hunger);
        }
    }
    ...
    assert!(final_amount < 5.0 || final_amount > 5.0, 
            "Resource amount should have changed from initial 5.0. Final: {}", final_amount);
}
...
fn test_resource_consumption_event() {
    ...
    for i in 0..180 {  // 3 seconds
        sim.update(1.0 / 60.0);
        
        if i % 60 == 0 {
            let creature = &sim.world.creatures[&creature_entity];
            let resource = &sim.world.resources[&food_entity];
            println!("Frame {}: creature pos={:?}, hunger={}, state={:?}, resource amount={}", 
    ...
    assert!(initial_amount > 0.0, "Test setup: resource should have initial amount");
}
...
fn test_multiple_creatures_share_resources() {
    ...
    for i in 0..60 {  // Just check first second
        sim.update(1.0 / 60.0);
        ...
        if c1.needs.hunger < 0.8 || c2.needs.hunger < 0.8 {
            any_ate = true;
            println!("Eating detected at frame {}: c1={}, c2={}", i, c1.needs.hunger, c2.needs.hunger);
            break;
        }
    }
    
    assert!(any_ate, "At least one creature should have eaten from the shared resource");
}
...
fn test_creature_stops_eating_when_satisfied() {
    ...
    for i in 0..60 {  // Check first second
        sim.update(1.0 / 60.0);
        ...
        if creature.needs.hunger <= 0.1 && !matches!(creature.state, CreatureState::Eating) {
            ...
            println!("Creature satisfied at frame {}: hunger={}, state={:?}", i, creature.needs.hunger, creature.state);
            break;
        }
    }
    
    assert!(satisfied_and_stopped, "Creature should stop eating when hunger is satisfied");
}