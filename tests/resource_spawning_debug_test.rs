...
fn debug_resource_spawning_and_consumption() {
    ...
    println!("Initial creature state: {:?}", sim.world.creatures[&entity].state);
    println!("Initial needs - Hunger: {}, Thirst: {}", 
             sim.world.creatures[&entity].needs.hunger,
             sim.world.creatures[&entity].needs.thirst);
    
    // Run for a few seconds and monitor
    for i in 0..300 { // 5 seconds
        sim.update(1.0 / 60.0);
        
        if i % 60 == 0 || i < 10 { // Every second or first 10 frames
            ...
            println!("Frame {}: State={:?}, Pos={:?}, Hunger={:.2}, Resources: {} food, {} water",
                     i, creature.state, creature.position, creature.needs.hunger, food_count, water_count);
                     
            // Check if any food is near the creature
            if food_count > 0 {
                let nearest_food = sim.world.find_nearest_resource(
                    creature.position, 
                    ResourceType::Food, 
                    None
                );
                if let Some((food_entity, distance)) = nearest_food {
                    ...
                    println!("  Nearest food at {:?}, distance: {:.1}, amount: {:.1}",
        ...
        if matches!(sim.world.creatures[&entity].state, 
            ...
            for j in 0..60 {
                ...
                if j % 10 == 0 {
                    let creature = &sim.world.creatures[&entity];
                    println!("  Eating frame {}: Hunger={:.2}, State={:?}", 
    ...
    println!("Final needs - Hunger: {:.2}, Thirst: {:.2}", 
