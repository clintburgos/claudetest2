...
fn test_decision_system_with_many_updates() {
    ...
    println!("Initial state: {:?}, position: {:?}", initial_state, initial_pos);
    ...
    assert!(state_changed, "Creature should have made a decision after 1000 updates!");
}
...
fn test_timer_ticking() {
    ...
    for i in 0..1000 {
        ...
        if timer.just_finished() {
            println!("Timer triggered at frame {} (elapsed: {:.6}s)", i, time.elapsed_seconds());
            triggered = true;
            break;
        }
