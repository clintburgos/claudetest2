...
fn test_basic_simulation_flow() {
    ...
    println!(
        ...
        creature
            .needs
            .get_urgency(creature_simulation::simulation::needs::NeedType::Hunger)
    );
    ...
    let creature = &sim.world.creatures[&creature_entity];
    println!(
        "After update 1 - State: {:?}, Position: {:?}, Hunger: {}",
    ...
    for i in 2..=30 {
        ...
        println!(
            "Creature - State: {:?}, Position: {:?}, Hunger: {}",
            creature.state, creature.position, creature.needs.hunger
        );
    ...
    assert!(
        final_creature.needs.hunger < 0.8,
        "Creature should have eaten"
    );
}
