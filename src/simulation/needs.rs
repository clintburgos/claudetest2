...
impl Needs {
    ...
    pub fn update(
        &mut self,
        dt: f32,
        metabolism_multiplier: f32,
        env_factors: &EnvironmentalFactors,
    ) {
        // Apply base rates with metabolism
        let hunger_rate =
            self.rates.hunger_per_second * metabolism_multiplier * env_factors.hunger_multiplier;
        let thirst_rate =
            self.rates.thirst_per_second * metabolism_multiplier * env_factors.thirst_multiplier;
        let energy_rate = self.rates.energy_drain_per_second
            * metabolism_multiplier
            * env_factors.energy_multiplier;
