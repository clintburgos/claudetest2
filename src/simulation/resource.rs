...
mod tests {
    ...
    fn resource_creation() {
        let resource = Resource::new(Entity::new(1), Vec2::new(10.0, 20.0), ResourceType::Food);
    ...
    fn resource_consumption() {
        let mut resource = Resource::new(Entity::new(1), Vec2::ZERO, ResourceType::Water);
    ...
    fn resource_regeneration() {
        let mut resource =
            Resource::new(Entity::new(1), Vec2::ZERO, ResourceType::Food).with_amount(40.0); // Start below max to allow regeneration

        resource.regenerate(10.0); // 10 seconds
        let expected = 40.0 + FOOD_REGENERATION_RATE * 10.0;
        assert!(
            (resource.amount - expected).abs() < 0.01,
            "Expected {}, got {}",
            expected,
            resource.amount
        );
    ...
    fn resource_type_properties() {
        ...
        assert_eq!(
            ResourceType::Food.regeneration_rate(),
            FOOD_REGENERATION_RATE
        );
        assert_eq!(
            ResourceType::Water.regeneration_rate(),
        ...
        assert_eq!(
            ResourceType::Water.consumption_rate(),
        ...
        assert_eq!(
            ResourceType::Water.default_max_amount(),
    ...
    fn resource_with_amount() {
        let resource =
            Resource::new(Entity::new(1), Vec2::ZERO, ResourceType::Food).with_amount(25.0);
        ...
        // Test clamping
        let resource2 =
            Resource::new(Entity::new(2), Vec2::ZERO, ResourceType::Water).with_amount(300.0);
    ...
    #[test]
    fn resource_is_full() {
        let mut resource = Resource::new(Entity::new(1), Vec2::ZERO, ResourceType::Food);
