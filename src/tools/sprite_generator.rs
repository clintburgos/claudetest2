...
pub fn generate_sprite_atlases() -> Result<(), Box<dyn std::error::Error>> {
    // Create directories
...
fn generate_creature_atlas() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (anim_idx, (_name, color)) in animations.iter().enumerate() {
        ...
        for frame in 0..8 {
            ...
            // Add genetic variation indicators
            if frame < 4 {
                // Size variation - draw outer ring
                draw_circle_outline(&mut atlas, x_offset + 24, y_offset + 24, 18, *color);
            } else {
                // Pattern variation - add spots
                draw_circle(&mut atlas, x_offset + 15, y_offset + 15, 3, darken_color(*color));
                draw_circle(&mut atlas, x_offset + 33, y_offset + 30, 3, darken_color(*color));
            }
        }
    }
...
// Binary to generate sprites
#[cfg(feature = "sprite-gen")]
fn main() {
    if let Err(e) = generate_sprite_atlases() {
        eprintln!("Error generating sprites: {}", e);
        std::process::exit(1);
    }
}