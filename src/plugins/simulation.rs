...
fn decision_system(
    ...
    resources: Query<(Entity, &Position, &ResourceTypeComponent, &ResourceAmount), With<ResourceMarker>>,
) {
    for (entity, mut timer, mut target, mut state, pos, vel, needs, health, creature_type) in creatures.iter_mut() {
        ...
        for &nearby_entity in &nearby_entities {
            ...
            if let Ok((creature_entity, creature_pos, creature_health, other_type)) = all_creatures.get(nearby_entity) {
                ...
                let is_threat = match (creature_type, other_type) {
                    ...
                    (CreatureType::Herbivore, CreatureType::Omnivore) => creature_health.current > health.current * 1.5,
                    _ => false,
                };
                ...
                if is_threat {
                    ...
                    nearby_creatures.push((creature_entity, creature_pos.0, distance, creature_health.current));
                }
            }
            
            // Check if it's a resource
            if let Ok((resource_entity, resource_pos, resource_type, amount)) = resources.get(nearby_entity) {
                if !amount.is_depleted() {
                    let distance = (pos.0 - resource_pos.0).length();
                    nearby_resources.push((resource_entity, resource_pos.0, resource_type.0, distance, amount.current));
                }
            }
        }
        ...
        if urgency > 0.7 {
            match need_type {
                crate::components::NeedType::Hunger => {
                    // Find nearest food
                    if let Some((food_entity, food_pos, _, distance, _)) = nearby_resources.iter()
                        .filter(|(_, _, res_type, _, _)| *res_type == crate::simulation::ResourceType::Food)
                ...
                crate::components::NeedType::Thirst => {
                    // Find nearest water
                    if let Some((water_entity, water_pos, _, distance, _)) = nearby_resources.iter()
                        .filter(|(_, _, res_type, _, _)| *res_type == crate::simulation::ResourceType::Water)
                        .min_by(|a, b| a.3.partial_cmp(&b.3).unwrap_or(std::cmp::Ordering::Equal))
                    {
                        if *distance < 2.0 {
        ...
        if needs.social > 0.6 && !nearby_creatures.is_empty() {
            if let Some((_, creature_pos, distance, _)) = nearby_creatures.iter()
                .min_by(|a, b| a.2.partial_cmp(&b.2).unwrap_or(std::cmp::Ordering::Equal))
            {
                if *distance > 10.0 {
                    *state = CreatureState::Moving { target: *creature_pos };
        ...
        if matches!(*state, CreatureState::Idle) {
            let wander_target = pos.0 + Vec2::new(
                (pos.0.x * 12.345).sin() * 50.0,
                (pos.0.y * 67.890).cos() * 50.0,
            );
            *state = CreatureState::Moving { target: wander_target };
...
fn movement_system(
    ...
    for (mut pos, mut vel, target, max_speed, state) in creatures.iter_mut() {
        ...
        if matches!(state, CreatureState::Dead | CreatureState::Eating | CreatureState::Drinking | CreatureState::Resting) {
            vel.0 = Vec2::ZERO;
            continue;
        }
...
fn consumption_system(
    ...
    mut resources: Query<(&Position, &mut ResourceAmount, &ResourceTypeComponent), With<ResourceMarker>>,
