...
use crate::core::{Entity, EntityManager, ErrorBoundary, EventBus, GameTime, SpatialGrid};
...
mod tests {
    ...
    fn world_entity_counts() {
        ...
        world.resources.insert(
            e3,
            Resource::new(e3, Vec2::ZERO, crate::simulation::ResourceType::Food),
        );
    ...
    fn world_clear() {
        ...
        world.resources.insert(
            e2,
            Resource::new(e2, Vec2::ZERO, crate::simulation::ResourceType::Water),
        );
        world.spatial_grid.insert(e1, Vec2::new(5.0, 5.0));
        world.spatial_grid.insert(e2, Vec2::new(10.0, 10.0));
        world.events.emit(crate::core::GameEvent::CreatureSpawned {
            entity: e1,
            position: Vec2::ZERO,
        });
    ...
    fn world_find_resources_near() {
        ...
        let nearby_food = world.find_resources_near(
            Vec2::new(10.0, 10.0),
            10.0,
            crate::simulation::ResourceType::Food,
        );
    ...
    fn world_find_nearest_resource() {
        ...
        let nearest =
            world.find_nearest_resource(Vec2::ZERO, crate::simulation::ResourceType::Food, None);
        assert!(nearest.is_some());
        assert_eq!(nearest.unwrap().0, food1);

        // Find nearest with max radius that excludes both
        let nearest_limited = world.find_nearest_resource(
            Vec2::ZERO,
            crate::simulation::ResourceType::Food,
            Some(10.0),
        );
        ...
        let no_water =
            world.find_nearest_resource(Vec2::ZERO, crate::simulation::ResourceType::Water, None);
    ...
    fn world_find_depleted_resources() {
        ...
        let nearby = world.find_resources_near(
            Vec2::new(10.0, 10.0),
            20.0,
            crate::simulation::ResourceType::Food,
        );
    ...
    fn world_update_stats() {
        ...
        world.resources.insert(
            e2,
            Resource::new(e2, Vec2::ZERO, crate::simulation::ResourceType::Food),
        );
